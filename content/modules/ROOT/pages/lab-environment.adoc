...
TIP: In this lab, there's no direct way to open a second terminal. However, when connected on bastion you can use `tmux`, which comes pre-installed with some user-friendly customizations to enhance your experience. To start `tmux`, simply type `tmux` in the bastion terminal.
